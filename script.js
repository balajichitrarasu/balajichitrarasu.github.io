...
(function () {
  ...
  window.addEventListener('load', function () {
    /* If we arrived with a hash, scroll to that section after transition */
    setTimeout(function () {
      document.body.classList.remove('page-loading');
      /* Scroll to hash after page-in animation */
  ...
  /* On ALL link clicks — animate out then navigate */
  document.addEventListener('click', function (e) {
    ...
    /* External links, resume, blank targets — let them open normally */
    if (a.target === '_blank' || href.indexOf('://') !== -1 || href === 'resume.pdf' || href.startsWith('mailto:') || href.startsWith('tel:')) return;

    /* Same-page hash anchors — smooth scroll with menu close, NO page reload */
    if (href.startsWith('#')) {
      e.preventDefault();
      var id = href.slice(1);
      if (!id) return;
      ...
      if (toggle2)  { toggle2.classList.remove('open'); toggle2.setAttribute('aria-expanded','false'); }
      setTimeout(function () {
    ...
    /* Cross-page links (blog.html, index.html) — page transition exit animation */
    e.preventDefault();
...
window.addEventListener('load', function () {
  ...
  /* ── 8. HAMBURGER MENU with X animation ── */
